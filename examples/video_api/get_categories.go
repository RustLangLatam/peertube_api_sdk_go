...
func main() {
	...
	// Fetch the categories list using the API client.
	// The context.Background() is used to create a background context for the request.
	video, response, err := apiClient.VideoAPI.GetCategories(context.Background()).Execute()

	// Check if the request failed or the response status code is not 200.
	if err != nil || response.StatusCode != 200 {
		log.Fatalf("Failed to fetch categories: %v (status code: %d)", err, response.StatusCode)
	}

	// Marshal the categories object into indented JSON.
	videoJSON, err := json.MarshalIndent(video, "", "  ")
	if err != nil {
		log.Fatalf("Failed to marshal categories to JSON: %v", err)
	}

	// Print the categories details in indented JSON format.
